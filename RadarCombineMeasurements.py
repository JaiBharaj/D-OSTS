...
def combine_radar_measurements(radars, true_traj):
    ...
    for i, time in enumerate(times):
        ...
        noise = []
        for rad in radars:
            is_visible = rad.satellite_measurements['visibility'][i].copy()
            if not is_visible:
                continue
            seen_radius.append(rad.satellite_measurements['r'][i].copy())
            noise.append(rad.get_noise()[i].copy())
        seen_radius = np.array(seen_radius)
        noise = np.array(noise)
        average_radius = weighted_average(seen_radius, noise)
        r_arr[i] = average_radius
